...
/**
 * Vendored from React Bits (https://reactbits.dev) — LaserFlow, JS + CSS variant, ported to
 * TypeScript. Shader math (VERT/FRAG) and the render/animation logic are unchanged from the
 * original; only types were added and the CSS became a module (see LaserFlow.module.css).
 *
 * Renders a raw-shader WebGL "laser" volume (beam + wisps + volumetric fog) sized to its
 * parent. Self-contained: creates its own <canvas>, pauses when off-screen/tab-hidden, adapts
 * its resolution to the measured frame rate, and cleans itself up on unmount.
 *
 * Not used directly by pages — see src/components/sections/OrderGlow.tsx, which lazy-loads
 * this component, gates it by device capability, and applies this site's own colors.
 */
...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      // Imperative canvas styling via a ref, deliberately outside React's render model (this
      // canvas is a plain DOM node owned by three.js, not reconciled) — not a render-purity issue.
